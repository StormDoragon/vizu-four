...
- [x] ~~[#9](https://github.com/StormDoragon/vizu-four/issues/9) **show intermediate evaluation steps in the expression playground**~~ — shipped, built directly on the existing lexer/parser/evaluator as the issue's notes anticipated (no evaluation-logic changes). A new `src/lib/expressions/trace.ts` mirrors `evaluator.ts`'s node dispatch to build a full sub-expression tree instead of just a final value - kept as a genuinely separate, drift-guarded module rather than a modified `evalNode`, so the production evaluator that the real engine's `if:`/output resolution depends on is untouched; a 41-fixture agreement test cross-checks every traced result against the real `evaluateExpression` for the same input to catch any future divergence immediately (the lesson from #11's three drifted color maps, but with a test this time). Every sub-expression shows its own value, in evaluation order; `matrix.node`/`steps.*.outputs.result`-style chains are tagged with the exact context path they resolve to; a `==`/`!=`/relational comparison whose operand types differ gets a coercion note (e.g. `number vs string: compared as numbers (1 == 1)`); the untaken side of `&&`/`||` is shown as "not evaluated — short-circuited" rather than inventing a result for code that GitHub's real short-circuit semantics never run; and a failure inside a nested call (`contains(fromJSON(badJson), x)`) is flagged exactly at the `fromJSON(...)` node that caused it, with only a lightweight "↳ failed below" marker on its ancestors - not a generic whole-expression error. A syntax error (parse failure, no AST at all) instead gets a `^` caret under the exact character position. The trace is masked through the same `maskObjectStrings` as the plain result, since a node can itself be `secrets.TOKEN`.
- [x] ~~[#8](https://github.com/StormDoragon/vizu-four/issues/8) **shareable session links**~~ — shipped, without any server-side storage: a link is a self-contained, URL-safe base64 token (`src/lib/share.ts`) holding the workflow YAML, breakpoints, What-If env/var overrides, mocked step outputs, and per-lane execution progress. The server discards the raw YAML once parsed, so it's cached client-side by content hash (`workflowSourceCache.ts`, same key `SessionView.workflowHash` already exposes) the moment a session is created from the home page - the one place the app ever sees that text - and read back when "Share" is clicked. Opening a link (`/share/[token]`) creates a brand-new session from that YAML, then replays it into the shared state by calling the same control/breakpoint/What-If/mock APIs a real user would, not by injecting server state directly. Reconstructing "pointer position" for a multi-job workflow turned out to be the real work here: a downstream job's lane is `blocked` until its `needs` all reach a terminal status (exactly like normal execution), so replay tracks every lane that had progressed - not just the one being viewed - and retries whichever aren't blocked each pass, verified live against the `needs`-chain example (`lint → test → deploy`/`notify`, `test` failing on purpose): the reconstructed session's lane statuses, pointers, and breakpoint all matched the original exactly, confirmed via direct session-state comparison, not just visually. **Secrets are never included** - not values, not even names (stricter than the local `debugPrefs` persistence, since a share link can leave the browser entirely); a recipient who needs one re-enters it in their own What-If tab. A dismissible "Viewing a shared session" banner (`?shared=1`) makes the distinction from an own session unambiguous. When the workflow text isn't cached in the current browser (cleared storage, or a link opened somewhere the session wasn't created), the Share button explains why rather than silently failing.
...
- [x] ~~Deploy a public demo (Vercel or similar) with 4–5 preloaded example workflows ([#1](https://github.com/StormDoragon/vizu-four/issues/1))~~ — shipped at [vizu-four.onrender.com](https://vizu-four.onrender.com), `VIZU_DEMO_MODE=1`. Initially deployed to Vercel, but its serverless model routes requests across multiple Lambda instances, each with its own empty copy of the in-memory session store (`src/lib/engine/store.ts`) — sessions could intermittently "disappear" mid-debug. Moved to Render's free tier, which runs exactly one persistent instance ([`render.yaml`](./render.yaml), see [DEPLOY.md](./DEPLOY.md#render-quickstart-recommended-free-one-instance-no-card)); the Vercel project is paused, not deleted, in case it's needed again.
...
- [x] "Share session" link (serialize workflow + current debug state) ([#8](https://github.com/StormDoragon/vizu-four/issues/8))
- [x] Expression playground improvements: show intermediate evaluation steps ([#9](https://github.com/StormDoragon/vizu-four/issues/9))
